...
public class ImportUtil {
    ...
    public static void addColumnValue(DbType dbType, StringBuffer buffer, int rowIndex,
            String cname, String cvalue, Column column) throws Exception {
        Object formatedValue = getSQLFormatedValue(dbType, rowIndex, cname, cvalue, column, false);
        buffer.append(formatedValue.toString());
    }

    public static Object getSQLFormatedValue(DbType dbType, int rowIndex, String cname,
            String cvalue, Column column, boolean bulkinsert) throws Exception {
            ...
            if (cvalue == null || "".equals(cvalue)) {
                if (!column.isNullable()) {
                    throw new SQLException(
                            "Error Row[" + rowIndex + "] the value of column[" + cname
                                    + "] should not be null!");
                }
                if (bulkinsert) {
                    return null;
        ...
        if (!bulkinsert && cvalue.contains("'")) {
            ...
            log.error(
                    "Error Row[" + rowIndex + "] contains single quotation marks in column[" + cname
        ...
        if ((cname.contains("_TEL") || cname.contains("PHONE") || cname.contains("MOBILE"))
            ...
            if (!rvalue.equals(cvalue)) {
                log.warn("Error Row[" + rowIndex
                        + "] wrong tel/mobile/phone number format, change it from [" + cvalue
                        + "] to [" + rvalue + "]");
                cvalue = rvalue;
            }
        }

        if ("NUMBER".equals(type) || "INT".equals(type) || "DECIMAL".equals(type) || "NUMERIC"
                .equals(type)) {
            /* previous update */
            if (cvalue.endsWith("%")) {
            ...
            if (!rvalue.equals(cvalue)) {
                log.warn(
                        "Error Row[" + rowIndex + "] wrong number format, change it from [" + cvalue
                                + "] to [" + rvalue + "]");
                cvalue = rvalue;
            }
            ...
            if (cvalue.length() > column.size) {
                throw new SQLException("Error Row[" + rowIndex + "] the size of the value[" + cvalue
                        + "] of column[" + cname + "] is " + cvalue.length()
                        + ", while the max size is " + column.size);
            }
            ...
            if (dotIndex > column.getIntegerSize()) {
                throw new SQLException(
                        "Error Row[" + rowIndex + "] the integer size of the value[" + cvalue
                                + "] of column[" + cname + "] is " + dotIndex
                                + ", while the integer max size is " + column.getIntegerSize());
            }
            if ("NUMBER".equals(column) && !cvalue.matches(NUMBER_PATTER)) {
                throw new SQLException(
                        "Error Row[" + rowIndex + "] the value[" + cvalue + "] is not a number!");
            }
            if ("INT".equals(column) && !cvalue.matches(INT_PATTERN)) {
                throw new SQLException(
                        "Error Row[" + rowIndex + "] the value[" + cvalue + "] is not a integer!");
            }
        ...
        /* CHAR,VARCHAR,NVARCHAR */
            ...
            if (cvalue.length() > column.size) {
                throw new SQLException(
                        "Error Row[" + rowIndex + "] the length of the value[" + cvalue
                                + "] of column[" + cname + "] is " + cvalue.length()
                                + ", while the max length is " + column.size);
            }
            if (bulkinsert) {
                return cvalue;
            }
    ...
    private static Object formateDate(DbType dbType, boolean bulkinsert, int rowIndex, String type,
        ...
        try {
            ...
            try {
                ...
                throw new RuntimeException(
                        "Error Row[" + rowIndex + "] bad date format value[" + cvalue
                                + "] for column " + cname);
            }
        }
    ...
    private static Object formateTimestampe(DbType dbType, boolean bulkinsert, int rowIndex,
            String type, String cname, String cvalue) {
        boolean firstFormat = true;
        try {
            ...
            try {
                ...
                throw new RuntimeException(
                        "Error Row[" + rowIndex + "] bad date time format value[" + cvalue
                                + "] for column " + cname);
            }
        }
        if (dbType.equals(DbType.oracle)) {
    ...
    public static void executeSqls(Connection connection, List<String> sqls) throws SQLException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            try {
                ...
                errorCount++;
                if (OracleInsertUtil.hasPkFkError(e.getMessage()) && ImportGlobals
                        .isContinueWhenPkFkError()) {
        ...
        log.info("success count:" + successCount + ", error count:" + errorCount);
        log.info("================================");
    ...
    public static void bulkinsetImp(DbType dbType, Connection connection, String userName,
            String tableName, List<String> columns, Map<String, Column> columnMap,
        ...
        for (int recordIndex = 0; recordIndex < dataHolder.getSize(); recordIndex++) {
            ...
            for (int i = 0; i < columns.size(); i++) {
                ...
                Object value = ImportUtil
                        .getSQLFormatedValue(dbType, recordIndex, cname, cvalue, column, true);
            ...
            if (primaryKeyValueSet.contains(primaryKeyValue)) {
                String errorMessage =
                        "Row[" + recordIndex + "] duplicated primay key value:" + primaryKeyValue;
        ...
        log.info(
                "correct data count:" + exactDataIndex + ", error data count:" + errorDataCount);
    ...
    public static void onebyoneInsertImp(DbType dbType, Connection connection, String tableName,
            List<String> columns, Map<String, Column> columnMap, DataHolder dataHolder)
            throws Exception {
    ...
    public static boolean validateData(DbType dbType, Connection connection, String userName,
            String tableName, List<String> columns, Map<String, Column> columnMap,
            DataHolder dataHolder) throws Exception {
        Set<String> primaryKeys = JdbcUtil.getPrimaryKeys(connection, userName, tableName);
        Set<String> primaryKeyValueSet = new HashSet<String>();
    ...
    public static String buildInsertSqlPrefix(String tableName, List columns,
            Map<String, Column> columnMap) throws Exception {
    ...
    private static void assertColumnExist(Map<String, Column> columnMap, String tableName,
